...
1. 将 `XyCore-0.3.1.jar` 放入 `plugins` 文件夹。
...
modules:
  ...
  # Kit：礼包/新手包模块预留开关。
  # 当前 0.3.1 暂未实现，开启也不会生成模块配置。
  kit: false

  # Nickname：昵称模块预留开关。
  # 当前 0.3.1 暂未实现，后续可用于玩家昵称、称号显示等功能。
  nickname: false

  # Script：脚本模块预留开关。
  # 当前 0.3.1 暂未实现，后续可用于轻量脚本、事件监听、命令注入等扩展。
  script: false
```
